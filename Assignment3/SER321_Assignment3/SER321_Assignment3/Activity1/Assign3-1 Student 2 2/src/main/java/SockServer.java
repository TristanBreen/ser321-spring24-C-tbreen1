...
public class SockServer {
  ...
  static List<storyboardEntry> storyboardEntries = new ArrayList<>();
  ...
  public static class storyboardEntry 
  {
    private final String username;
    private final String sentence;

    public storyboardEntry(String username, String sentence) 
    {
    ...
    public static String listToString(List<storyboardEntry> entries) 
    {
        StringBuilder stringBuilder = new StringBuilder();
        for (storyboardEntry entry : entries) 
        {
            stringBuilder.append(entry.getUsername()).append(": ").append(entry.getSentence()).append("\n");
        }
  ...
  static JSONObject storyboard(JSONObject req) {
    ...
    JSONObject res = testField(req, "story");
    res.put("type", "storyboard");
     
      
    if(String.valueOf(req.get("view")).equals("false"))
    {
      String user = String.valueOf(req.get("name"));
      String text = String.valueOf(req.get("story"));
        
      for (storyboardEntry entry : storyboardEntries) 
      {
        if (entry.getUsername().equals(String.valueOf(req.get("name")))) 
        {
          res.put("ok", false);
          res.put("message", "\nThat username already has an entry\n");
          return res;
        }
      }

      storyboardEntries.add(new storyboardEntry(user,text));
      res.put("result", "\nStory Recieved\n");
    }
    else
    {
      //view
      String entries = storyboardEntry.listToString(storyboardEntries);
      res.put("result", "\n" + entries);
    }
  ...
  static JSONObject charCount(JSONObject req) {
    ...
    if (res.getBoolean("ok")) {
      ...
      int count = 0;
      char charCount = '%';
      if(!String.valueOf(req.get("findchar")).equals("false") && String.valueOf(req.get("findchar")).length() == 1)
      {
        charCount = String.valueOf(req.get("findchar")).charAt(0);
      }
      for(int i = 0; i < String.valueOf(req.get("count")).length(); i++)
      {
        if(charCount != '%' && charCount == String.valueOf(req.get("count")).toLowerCase().charAt(i))
        {
          count++;
        }
        if(charCount == '%')
